...
void DataFormatter::Colorize(const uvc_frame_t *input, QVideoFrame &output) const
{
    ...
    switch(m_pseudocolor_palette)
    {
    case Palette::IronBlack:
        palette = colormap_ironblack;
        break;
    case Palette::Rainbow:
        palette = colormap_rainbow;
        break;
    case Palette::Grayscale:
    default:
        palette = colormap_grayscale;
        break;
    }
    ...
    for (uint32_t i = 0; i < input->height; i++)
    {
        ...
        for (uint32_t j = 0; j < input->width; j++)
        {
            ...
            rgba_line[j * 4 + 0] = rgb[0];
            rgba_line[j * 4 + 1] = rgb[1];
            rgba_line[j * 4 + 2] = rgb[2];
            rgba_line[j * 4 + 3] = 0;
        }
    }
    output.unmap();
